...
static void rtmp_custom_update(void *data, obs_data_t *settings)
{
	...
	bfree(service->key);
	//这里设置直播的信息
	service->server = bstrdup("http://192.168.1.6/hls");// bstrdup(obs_data_get_string(settings, "server"));
	service->key = bstrdup("movie1234");// bstrdup(obs_data_get_string(settings, "key"));
