		...
		const char *oldName = nullptr)
{
	...
	ret = GetConfigPath(path, sizeof(path), "zblive-app/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
		...
		const char *init_text)
{
	...
	char newPath[512];
	int ret = GetConfigPath(newPath, 512, "zblive-app/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
...
void OBSBasic::DeleteProfile(const char *profileName, const char *profileDir)
{
	...
	int ret = GetConfigPath(basePath, 512, "zblive-app/basic/profiles");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return;
	}
...
void OBSBasic::on_actionImportProfile_triggered()
{
	...
	int ret = GetConfigPath(path, 512, "zblive-app/basic/profiles/");
	if (ret <= 0) {
