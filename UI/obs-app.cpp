...
bool OBSApp::InitGlobalConfigDefaults()
{
	...
	config_set_default_double(globalConfig, "BasicWindow",
			"SnapDistance", 10.0);
	config_set_default_bool(globalConfig, "BasicWindow",
			"RecordWhenStreaming", true);//默认推流时自动录像
	config_set_default_bool(globalConfig, "BasicWindow",
...
static bool MakeUserDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "zblive-app/basic") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "zblive-app/logs") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "zblive-app/profiler_data") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

#ifdef _WIN32
	if (GetConfigPath(path, sizeof(path), "zblive-app/crashes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "zblive-app/updates") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
#endif

	if (GetConfigPath(path, sizeof(path), "zblive-app/plugin_config") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
static bool MakeUserProfileDirs()
{
	...
	if (GetConfigPath(path, sizeof(path), "zblive-app/basic/profiles") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;

	if (GetConfigPath(path, sizeof(path), "zblive-app/basic/scenes") <= 0)
		return false;
	if (!do_mkdir(path))
		return false;
...
bool OBSApp::InitGlobalConfig()
{
	...
	//csong added  默认推流时录像
	if (!config_has_user_value(globalConfig, "BasicWindow", "RecordWhenStreaming")) {
		
		config_set_bool(globalConfig, "BasicWindow", "RecordWhenStreaming",
			true);
		changed = true;
	}
...
static void move_basic_to_profiles(void)
{
	...
	if (GetConfigPath(path, 512, "zblive-app/basic") <= 0)
		return;
	if (!os_file_exists(path))
		return;

	/* if the profiles directory doesn't already exist */
	if (GetConfigPath(new_path, 512, "zblive-app/basic/profiles") <= 0)
		return;
...
static void move_basic_to_scene_collections(void)
{
	...
	if (GetConfigPath(new_path, 512, "zblive-app/basic/scenes") <= 0)
		return;
	if (os_file_exists(new_path))
		return;
...
{
	BPtr<char>       logDir(GetConfigPathPtr("zblive-app/logs"));
	struct os_dirent *entry;
	os_dir_t         *dir        = os_opendir(logDir);
...
static void create_log_file(fstream &logFile)
{
	...
	dst << "zblive-app/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file("zblive-app/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("zblive-app/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		delete_oldest_file("zblive-app/profiler_data");
		...
		if (already_running && !multi) {
			blog(LOG_WARNING, "\n================================");
			blog(LOG_WARNING, "Warning: ZBLive is already running!");
			blog(LOG_WARNING, "================================\n");
...
#define CRASH_MESSAGE \
	...
	"%appdata%\\zblive-app\\crashes directory)"
...
static void main_crash_handler(const char *format, va_list args, void *param)
{
	...
	delete_oldest_file("zblive-app/crashes");

	string name = "zblive-app/crashes/Crash ";
...
static void move_to_xdg(void)
{
	...
	if (snprintf(old_path, 512, "%s/.zblive-app", home) <= 0)
		return;
	...
	if (GetConfigPath(new_path, 512, "zblive-app") <= 0)
		return;
...
{
	char path[512];
	int pathlen = GetConfigPath(path, 512, "zblive-app/basic/profiles");
