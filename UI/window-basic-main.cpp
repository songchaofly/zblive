...
static void AddExtraModulePaths()
{
	...
	BPtr<char> config_bin = os_get_config_path_ptr("zblive-app/plugins/%module%/bin");
	BPtr<char> config_data = os_get_config_path_ptr("zblive-app/plugins/%module%/data");
...
bool OBSBasic::InitBasicConfigDefaults()
{
	...
	config_set_default_string(basicConfig, "SimpleOutput", "FilePath", "d:\\中博智学-直播录屏");//csong added GetDefaultVideoSavePath().c_str());
	...
	config_set_default_string(basicConfig, "AdvOut", "RecFilePath", "d:\\中博智学-直播录屏");//csong added	GetDefaultVideoSavePath().c_str());
	...
	config_set_default_bool  (basicConfig, "AdvOut", "FFOutputToFile",
			true);
	config_set_default_string(basicConfig, "AdvOut", "FFFilePath", "d:\\中博智学-直播录屏");//csong added GetDefaultVideoSavePath().c_str());
...
void OBSBasic::OBSInit()
{
	...
	ret = snprintf(fileName, 512, "zblive-app/basic/scenes/%s.json",
			sceneCollection);
	if (ret <= 0)
...
{
	char logDir[512];
	if (GetConfigPath(logDir, sizeof(logDir), "zblive-app/logs") <= 0)
		return nullptr;
...
{
	char path[512];
	int ret = GetConfigPath(path, 512, "zblive-app");
	if (ret <= 0)
		return;
...
int OBSBasic::GetProfilePath(char *path, size_t size, const char *file) const
{
	...
	ret = GetConfigPath(profiles_path, 512, "zblive-app/basic/profiles");
	if (ret <= 0)
		return ret;
